...
async def universal_child_name_handler(message: types.Message):
    """Handles pending child name input."""
    tg_id = message.from_user.id
    if tg_id not in pending_child_name:
        return
    text = message.text.strip()
    if len(text) < 3 or len(text) > 10 or not text.isalpha():
        await message.answer("❌ Имя: 3–10 букв!")
        return
    session = pending_child_name.pop(tg_id)
    await db.add_child(session["marriage_id"], text, session["gender"])
    await message.answer(f"👶 <b>{text}</b> ({session['gender']}) добавлен в семью!", parse_mode="HTML")
